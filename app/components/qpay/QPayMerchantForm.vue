...
import type { RegisterCompanyRequest, RegisterPersonRequest, QPayLocation } from '~/composables/useQPay'
...
const { registerCompanyMerchant, registerPersonMerchant, isRegistering, fetchCities, fetchDistricts } = useQPay()
const { user } = useAuth()
...
const isBankComplete = computed(() => !!(bankCode.value && bankAccountNumber.value && bankAccountName.value))
  ...
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between px-1 py-2">
      ...
      <div class="flex gap-1">
        <div
          ...
          :class="i <= completedSectionsCount ? 'bg-primary-500' : 'bg-gray-200 dark:bg-gray-700'"
        />
      </div>
    </div>
    ...
    <UAccordion
      ...
      :items="[
        ...
        { value: 'contact', label: 'Холбоо барих', icon: 'i-lucide-phone', slot: 'contact' as const },
        { value: 'address', label: 'Хаяг', icon: 'i-lucide-map-pin', slot: 'address' as const },
        { value: 'bank', label: 'Банкны данс', icon: 'i-lucide-credit-card', slot: 'bank' as const }
      ]"
      class="space-y-2"
    >
      ...
      <template #trailing="{ item }">
        <UIcon
          v-if="(item.value === 'personal-info' && isPersonalInfoComplete)
        ...
        <div class="space-y-4 pt-2">
          ...
          <template v-if="merchantType === 'person'">
            <UFormField
              ...
              required
              hint="Жишээ: АА12345678"
            >
              <UInput
                v-model="personRegisterNo"
          ...
          <template v-else>
            ...
            >
              <UInput
                v-model="companyOwnerRegisterNo"
                placeholder="АА12345678"
              />
            </UFormField>
