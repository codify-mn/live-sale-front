...
<template>
  <DashboardCard padding="none">
    ...
    <div v-else-if="orders.length === 0" class="p-8 text-center">
      <div class="w-14 h-14 mx-auto rounded-2xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center mb-3">
    ...
    <div v-else class="divide-y divide-gray-100 dark:divide-gray-800/50">
      ...
      >
        ...
        <div
          ...
          :class="{
            'bg-amber-100/80 dark:bg-amber-900/20 text-amber-500': order.status === 'pending',
            'bg-green-100/80 dark:bg-green-900/20 text-green-500': order.status === 'paid' || order.status === 'delivered',
            'bg-blue-100/80 dark:bg-blue-900/20 text-blue-500': order.status === 'shipped',
            'bg-red-100/80 dark:bg-red-900/20 text-red-500': order.status === 'cancelled',
            'bg-gray-100/80 dark:bg-gray-800/50 text-gray-500': order.status === 'refunded'
          }"
        >
          <UIcon :name="statusIcons[order.status] || 'i-lucide-package'" class="w-5 h-5" />
        </div>
