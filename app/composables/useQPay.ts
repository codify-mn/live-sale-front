...
const _useQPay = () => {
  ...
  const getOrderInvoice = async (orderId: number) => {
    try {
      const data = await $fetch<QPayInvoice>(`${config.public.apiUrl}/api/orders/${orderId}/qpay`, {
        credentials: 'include'
      })
      return data
    } catch (err: any) {
  ...
  const checkPayment = async (orderId: number) => {
    ...
    try {
      const data = await $fetch<PaymentCheckResult>(`${config.public.apiUrl}/api/qpay/payment/check`, {
        method: 'POST',
        credentials: 'include',
  ...
  const fetchDistricts = async (cityCode: string) => {
    try {
      const data = await $fetch<QPayLocation[]>(`${config.public.apiUrl}/api/qpay/districts/${cityCode}`, {
        credentials: 'include'
      })
      return data
    } catch (err: any) {
