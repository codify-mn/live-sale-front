...
export function useOrders() {
  ...
  const fetchRecentOrders = async (limit: number = 5): Promise<Order[]> => {
    const response = await $fetch<{ orders: Order[] }>(`${apiUrl}/api/orders/recent?limit=${limit}`, {
      credentials: 'include'
    })
  ...
  const getStatusColor = (status: OrderStatus): 'warning' | 'info' | 'success' | 'error' | 'neutral' => {
