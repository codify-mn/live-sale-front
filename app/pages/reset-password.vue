  ...
  <UCard class="max-w-sm w-full">
    ...
    <div class="space-y-4">
      ...
      <div v-if="isInvalidToken" class="text-center space-y-4">
        ...
        <UButton to="/forgot-password" variant="outline" block>
          Дахин холбоос авах
        </UButton>
      </div>
      ...
      <template v-else>
        ...
        <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
          <UFormField label="Шинэ нууц үг" name="password">
            <UInput v-model="state.password" type="password" placeholder="Хамгийн багадаа 8 тэмдэгт"
              icon="i-lucide-lock" size="lg" />
          </UFormField>

          <UFormField label="Нууц үг давтах" name="confirmPassword">
            <UInput v-model="state.confirmPassword" type="password" placeholder="Нууц үг давтана уу"
              icon="i-lucide-lock" size="lg" />
          </UFormField>
