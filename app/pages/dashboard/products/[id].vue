...
const onSubmit = async (event: FormSubmitEvent<Schema>) => {
    ...
    try {
        ...
        toast.add({ title: 'Амжилттай', description: 'Бүтээгдэхүүн шинэчлэгдлээ', color: 'success' })
        await loadProduct()
    } catch (err: any) {
        toast.add({ title: 'Алдаа', description: err.data?.message || 'Шинэчлэхэд алдаа гарлаа', color: 'error' })
    } finally {
        saving.value = false
    }
}
...
const confirmDelete = async () => {
    ...
    try {
        ...
        toast.add({ title: 'Алдаа', description: err.data?.message || 'Устгахад алдаа гарлаа', color: 'error' })
    } finally {
        ...
        <UDashboardPanel id="product-detail">
            ...
            <template #body>
                            ...
                            <div class="lg:col-span-2 space-y-6">
                                ...
                                <ProductFormCard title="Барааны төрлүүд">
                                    <div class="flex items-center gap-1 border-b border-gray-200 dark:border-gray-700 overflow-x-auto">
                                        <div
                                            v-for="(variant, index) in variants" :key="`tab-${index}`"
                                            class="group relative flex items-center gap-1.5 px-3 py-1.5 cursor-pointer border-b-2 min-w-[120px]"
                                            :class="activeTabIndex === index ? 'border-primary-500 text-primary-700' : 'border-transparent text-gray-600'"
                                            @click="activeTabIndex = index"
                                        >
                                            <span class="text-xs font-medium truncate flex-1">{{ variant.name || `Төрөл ${index + 1}` }}</span>
                                            <button v-if="variants.length > 1" @click.stop="removeVariant(index)"><UIcon name="i-lucide-x" class="w-3 h-3" /></button>
                                        </div>
                                        <button @click="addVariant" class="px-2 py-1.5 text-gray-500"><UIcon name="i-lucide-plus" class="w-3.5 h-3.5" /></button>
                                    </div>
                                    <div class="mt-4">
                                        <ProductVariantForm
                                            v-if="currentVariant" :model-value="currentVariant" :index="activeTabIndex"
                                            :product-name="state.name" @update:model-value="handleVariantUpdate(activeTabIndex, $event)"
                                            @remove="removeVariant(activeTabIndex)" @duplicate="duplicateVariant(activeTabIndex)"
                                        />
                                    </div>
                                </ProductFormCard>
                            </div>
                ...
                <UModal v-model:open="deleteModalOpen">
                    <template #content>
                        <UCard>
                            <p><strong>{{ product?.name }}</strong> устгахдаа итгэлтэй байна уу?</p>
                            <template #footer>
                                <div class="flex justify-end gap-2">
                                    <UButton color="neutral" @click="deleteModalOpen = false">Болих</UButton>
